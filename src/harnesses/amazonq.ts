 ...
 * Uses Claude-like format: { "mcpServers": { "name": { "command": "...", "args": [...], "env": {...} } } }
 * Project: .amazonq/mcp.json
 * Global: ~/.amazonq/mcp.json
 */
...
export const amazonq: HarnessAdapter = {
  name: 'amazonq',
  displayName: 'Amazon Q',

  detect(scope, projectDir) {
    if (scope === 'global') {
      return existsSync(join(homedir(), '.amazonq'));
    }
    return existsSync(join(projectDir, '.amazonq'));
  },

  configPath(scope, projectDir) {
    if (scope === 'global') {
      return join(homedir(), '.amazonq', 'mcp.json');
    }
    return join(projectDir, '.amazonq', 'mcp.json');
  },
